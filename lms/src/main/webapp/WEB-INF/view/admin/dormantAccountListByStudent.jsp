			...
			$(document).ready(function() {
				...
				$('button[name=ActiveBtn]').click(function() {
					...
					if(accountStateActive) {
						...
						location.replace('${pageContext.request.contextPath}/admin/dormantAccountStateActiveByStudent?accountId=' + activeAccountId + '&currentPage=' + currentPage + '&searchType=' + searchType + '&searchKeyword=' + encodeURI(searchKeyword, "UTF-8"));
				...
				<table class="table">
					...
					<tbody>
						...
						<c:if test="${lastPage != 0}">
							<c:forEach var="dormantAccountListByStudent" items="${dormantAccountListByStudent}">
								<tr>
									<td>${dormantAccountListByStudent.accountId}</td>
									<td>${dormantAccountListByStudent.studentName}</td>
									<td>${dormantAccountListByStudent.studentEmail}</td>
									<td>
										<button type="button" id="ActiveBtn" name= "ActiveBtn" class="btn btn-outline-success" value="${dormantAccountListByStudent.accountId}">
											활성화
										</button>
									</td>
								</tr>
