			...
			$(document).ready(function() {
				...
				$('button[name=ActiveBtn]').click(function() {
					...
					if(accountStateActive) {
						...
						location.replace('${pageContext.request.contextPath}/admin/dormantAccountStateActiveByTeacher?accountId=' + activeAccountId + '&currentPage=' + currentPage + '&searchType=' + searchType + '&searchKeyword=' + encodeURI(searchKeyword, "UTF-8"));
				...
				<table class="table">
					...
					<tbody>
						...
						<c:if test="${lastPage != 0}">
							<c:forEach var="dormantAccountListByTeacher" items="${dormantAccountListByTeacher}">
								<tr>
									<td>${dormantAccountListByTeacher.accountId}</td>
									<td>${dormantAccountListByTeacher.teacherName}</td>
									<td>${dormantAccountListByTeacher.teacherEmail}</td>
									<td>
										<button type="button" id="ActiveBtn" name= "ActiveBtn" class="btn btn-outline-success" value="${dormantAccountListByTeacher.accountId}">
											활성화
										</button>
									</td>
								</tr>
