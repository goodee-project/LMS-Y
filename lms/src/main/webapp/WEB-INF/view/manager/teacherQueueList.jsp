			...
			<form action="${pageContext.request.pathInfo}" method="get">
				<div class="input-group mb-3">
					<input type="text" class="form-control" placeholder="Search"
						name="teacherName" value="${teacherName}">
		...
		<table class="table">
			...
			<c:forEach var="tq" items="${teacherQueueList}">
				<tr>
					<td>${tq.accountId}</td>
					<td>
						<a href="${pageContext.request.contextPath}/manager/teacherQueueDetail?accountId=${tq.accountId}">${tq.teacherName}</a>
					</td>
						...
						<c:if test="${i <= lastPage}">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item">
										<a class="page-link"
										href="${pageContext.request.pathInfo}?currentPage=${i}&teacherName=${teacherName}">${i}</a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:if>
