...
<script>
      $(document).ready(function() {
         ...
         var imageCheck = '';
         // 이미지에 대한 제약조건 명시
         $('#TeacherImageFileFrame').change(function(){
         ...
         $('#createLectureArchiveFile').click(function() {
            ...
            $('#TeacherImageFileFrame').append(`
               <div class="input-group mb-3">
                  <input class="imageFileList" type="file" name="imageFileList" id="TeacherImageFileFrame"/>
                  <button class="removelectureArchiveFile btn btn-outline-danger" type="button">삭제</button>
               </div>
            `);
            
            // (바로 위의 코드에서 추가한) 삭제버튼에 대한 이벤트 처리를 등록함
            $('#TeacherImageFileFrame:last-child .removelectureArchiveFile').click(function(event) {
   ...
   <div class=container>
      <form class="" id="modifyTeacherForm" method="post" action="${pageContext.request.contextPath}/teacher/modifyTeacher" enctype="multipart/form-data">
         <table class="table">
            ...
            <tr>
               <td>프로필 사진</td>
               <td><img src="${map.imageURI}" name="teacherImage" id="Imageview" onerror="this.src='https://www.flaticon.com/svg/static/icons/svg/149/149071.svg';" style="width: 170px; height: 200px;" />
                  <c:if test="${not empty map.teacher.teacherImage}">
                  ...
                  <div id="TeacherImageFileFrame"></div>
               <td></td>
            </tr>
            <tr>
               <td></td>
